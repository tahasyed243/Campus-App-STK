...
const ApplicantsTable = ({ jobId }) => {
    ...
    const unsubscribe = onValue(jobRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        Object.keys(data).forEach((key) => {
          const applicant = { id: key, ...data[key] };
  ...
  return (
    <div>
      <Table>
        <TableCaption>List of applicants (Admin view)</TableCaption>
        <TableHeader>
          <TableRow>
                  ...
                  <PopoverContent className="w-32">
                    {shortlistingStatus.map((status) => (
                      <div
                        ...
                        className="flex w-fit items-center my-2 cursor-pointer hover:bg-gray-200 rounded-md p-1"
                      >
                        {status}
                      </div>
                    ))}
